...
class AdRemoverEngine(private val context: Context) {
    ...
    suspend fun processApp(
        ...
        try {
            ...
            onStateChange(ProcessState.Patching("正在移除广告代码..."))
            ...
            val unsignedApk = File(context.cacheDir, "${packageName}_unsigned.apk")
            val repackaged = patcher.repackageApk(extractedDir, unsignedApk)
            
            if (!repackaged) {
                throw Exception("APK打包失败")
            }
            
            onStateChange(ProcessState.Signing)
            val signedApk = File(context.cacheDir, "${packageName}_signed.apk")
            ...
            val finalApk = File(outputDir, "${appName}_no_ads.apk")
