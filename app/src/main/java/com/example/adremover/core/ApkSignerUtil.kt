...
class ApkSignerUtil {
    ...
    fun signApk(inputApk: File, outputApk: File): Boolean {
        return try {
            ...
            val certificate = cf.generateCertificate(certHolder.encoded.inputStream()) as java.security.cert.X509Certificate
            ...
            apkSigner.sign()
            Log.d(TAG, "Done, output size=`{outputApk.length()}")
            true
        } catch (e: Exception) {
